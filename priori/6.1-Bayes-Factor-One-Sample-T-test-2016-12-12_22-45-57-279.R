...
dz <- 0.156 #Enter Cohen's dz effect size observed in the dependent t-test or one-sided t-test

dz_prior <- 0.192 #Enter effect size dz for the prior 
sd_prior <- 10.05 #Enter sd of the effect sizes of the prior - the higher, the wider the prior is
...
png(file=paste('large_objects_prior.png'),width=6000,height=4000, res = 1000)
maxAlt=max(f(delta))
...
png(file=paste('large_objects_posterior.png'),width=6000,height=4000, res = 1000)
