...
RT_DF = data.frame(
          ...
          with(data = rbind(FILTER_EUR, FILTER_TCU), tapply(response_time, paste(ID, size, Pic, Match, sep = "-"), mean)),
          with(data = rbind(FILTER_EUR, FILTER_TCU), tapply(correct, paste(ID, size, Pic, Match, sep = "-"), table))/10,
        row.names = NULL
)
